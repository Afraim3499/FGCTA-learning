...
const L1_CONTENT = [
  {
    ...
    interactiveTaskType: "choice",
    interactiveTaskData: {
      question: "If price is currently moving sideways in a tight horizontal range, what does this tell you about market pressure?",
      options: [
  ...
  },
  {
    ...
    content: `# Market Participants\n\nThe market is a collection of diverse participants, each with different goals, tools, and capital sizes. Understanding these roles helps remove the mystery of how markets function.\n\n### Larger Participants\n\nFinancial institutions, central banks, and hedge funds are the "heavyweights." Their orders are so large that they cannot be filled instantly without moving the market significantly. Because of this, they often use **Resting Orders** to build their positions over time.\n\n![Participant Flow](/images/academy/l1/onb_02_participants.png)\n\n### Market Providers\n\nLiquidity providers (often large banks) act as the middlemen. Their primary role is to offer available prices to both buyers and sellers, ensuring that transactions can occur smoothly. They don't necessarily "predict" direction; they facilitate flow.\n\n### Retail Traders\n\nIndividual traders like us have the smallest capital but the highest speed. We can enter and exit the market instantly. Our goal is to identify the "footprints" left behind by larger participants and follow the path of least resistance.`,
    interactiveTaskType: "reveal",
    interactiveTaskData: {
      items: [
        { 
          title: "Institutional Motive", 
          content: "Larger participants act based on long-term value, hedging, or large-scale order fulfillment. They move slowly and leave clear footprints in the form of aggressive price expansion." 
        },
        { 
          title: "Provider Motive", 
          content: "Market providers earn from the spread (bid/ask difference) and volume. They ensure liquidity is always available for other participants." 
        },
        { 
          title: "Retail Motive", 
          content: "Retail traders seek to capitalize on price swings. Because we have zero market impact, our edge comes from following the momentum created by institutions." 
        }
      ]
    }
  },
  {
    ...
    interactiveTaskType: "identify",
    interactiveTaskData: {
      instruction: "Examine the chart. Click the area where price launched upward with the most aggressive speed.",
      regions: [
        { id: "demand", x: 20, y: 70, width: 15, height: 20, isCorrect: true, feedback: "Correct! This aggressive 'launch' marks a strong Demand Zone." },
        { id: "noise", x: 50, y: 40, width: 10, height: 10, isCorrect: false, feedback: "This is a ranging area, not a clear zone of imbalance." }
      ]
    }
  },
  {
    ...
    content: `# Liquidity Basics\n\nIn trading, **Liquidity** refers to the availability of orders. For a large participant to buy, there must be enough sellers. For them to sell, there must be enough buyers.\n\n### The Magnet Effect\n\nMany traders place their **Stop-Loss** or **Breakout Orders** around obvious highs and lows. These areas represent "Liquidity Pools." Larger participants are often attracted to these pools because they provide the volume necessary to fill their large positions.\n\n![Liquidity Clusters](/images/academy/l1/onb_04_liquidity.png)\n\n### Obvious Highs and Lows\n\n- **Buy-Side Liquidity**: Resting orders sitting above an obvious peak (swing high).\n- **Sell-Side Liquidity**: Resting orders sitting below an obvious trough (swing low).\n\nWhen price "sweeps" one of these areas, it often reverses because the resting orders have been consumed, and the imbalance has shifted.`,
    interactiveTaskType: "identify",
    interactiveTaskData: {
      instruction: "Look at the swing highs and lows. Click the area where you would expect the highest concentration of resting stop-loss orders.",
      regions: [
        { id: "high_liq", x: 45, y: 15, width: 20, height: 10, isCorrect: true, feedback: "Correct! Obvious peaks are magnets for stop-loss and breakout buy orders." },
        { id: "mid_range", x: 50, y: 50, width: 10, height: 10, isCorrect: false, feedback: "Orders are rarely clustered in the middle of a range." }
      ]
    }
  },
  {
    ...
    interactiveTaskType: "choice",
    interactiveTaskData: {
      question: "The market has been moving sideways for 4 hours during a quiet Asian session. No clear zones have been reached. What is the professional decision?",
      options: [
...
async function overhaul() {
  console.log("🚀 Starting Level 1 Foundation Pack Overhaul...");

  try {
    await prisma.$transaction(async (tx) => {
      // 1. Update Modules 1.1 - 1.5
      for (const mod of L1_CONTENT) {
        ...
        console.log(`  ✅ Updated Module ${mod.moduleNumber}: ${mod.title}`);
      }

      // 2. Prefix Legacy Modules 1.6 - 1.15
      const legacyModules = await tx.courseModule.findMany({
        where: {
          level: 1,
      ...
      for (const mod of legacyModules) {
        const newTitle = mod.title.startsWith("[Legacy]") ? mod.title : `[Legacy] ${mod.title}`;
        await tx.courseModule.update({
          where: { id: mod.id },
          data: { title: newTitle }
        });
        console.log(`  📦 Prefixed Legacy Module ${mod.moduleNumber}: ${newTitle}`);
      }

      // 3. Update Knowledge Test
      await tx.knowledgeTest.upsert({
        where: { level: 1 },
        update: L1_TEST,
        create: L1_TEST
      });
      console.log("  🏆 Updated Level 1 Knowledge Test");
    });
