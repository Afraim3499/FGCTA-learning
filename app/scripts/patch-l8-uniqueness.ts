...
function runPatch() {
  console.log("--- STARTING LEVEL 8 UNIQUENESS PATCH ---");

  // Phase 1: Track title frequencies across the entire Level 8
    ...
    for (const file of files) {
      ...
      try {
        ...
        for (const card of cards) {
          if (card.title) {
            const baseTitle = card.title.trim().replace(/\s\(Module 8\.\d+( - [A-Z]+)?\)$/i, "").toLowerCase();
  ...
  for (const mod of modules) {
    const modDir = path.join(level8Dir, mod);
    const modNum = mod.split("module-8-")[1];
    ...
    for (const file of files) {
      ...
      try {
        ...
        cards.forEach((card, index) => {
          const baseTitle = card.title.trim().replace(/\s\(Module 8\.\d+( - [A-Z]+)?\)$/i, "");
          ...
          if ((titleCounts.get(cleanTitle) || 0) > 1) {
            ...
            const newTitle = `${baseTitle} (Module 8.${modNum} - ${trackName})`;
          ...
          const comment = `<!-- M8.${modNum}-${file.replace(".ts", "")}-C${index + 1} -->\n`;
  ...
  console.log(`--- LEVEL 8 UNIQUENESS PATCH COMPLETED: Patched ${updatedFilesCount} files ---`);
}
