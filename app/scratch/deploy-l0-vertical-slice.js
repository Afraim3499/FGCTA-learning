...
async function main() {
  console.log('--- LEVEL 0 VERTICAL SLICE UPDATE ---');

  for (const data of MODULE_DATA) {
    console.log(`Processing Module ${data.moduleNumber}...`);
  ...
  console.log('Updating Mission 1...');
  await prisma.trainingScenario.update({
    where: { slug: 'l0-sync-order-entry-v1' },
    data: {
      prompt: "This is a safe order configuration test, not a real trade or profit challenge. Objective: Successfully execute an order with a valid Stop Loss, Take Profit (min 2R), and 0.01 lot size.",
      xpAward: 25,
      metadata: {
        maxLotSize: 0.01,
        lockInstrument: true,
  ...
  console.log('✅ Level 0 Vertical Slice Deployed.');
}
