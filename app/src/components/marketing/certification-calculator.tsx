      ...
      <div className="max-w-4xl mx-auto space-y-12">
        <div className="text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-indigo-500/10 border border-indigo-500/20">
            ...
            <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-indigo-400">Institutional Tool</span>
          </div>
          <h2 className="text-4xl md:text-5xl font-bold tracking-tighter">Certification Simulator</h2>
          <p className="text-[var(--color-text-secondary)] max-w-lg mx-auto">
            See how institutional metrics impact your score. Adjust the parameters to see what it takes to reach Level 5.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
              ...
              <div key={m.name} className="space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <m.icon size={14} className="text-[var(--color-text-muted)]" />
                ...
                <input
                  ...
                  className="w-full h-1.5 bg-white/10 rounded-full appearance-none cursor-pointer accent-[var(--color-brand-500)]"
                />
                <p className="text-[10px] text-[var(--color-text-muted)] italic">{m.description}</p>
              </div>
            ))}
          </div>
          ...
          <div className="relative group">
            <div className="absolute -inset-1 bg-gradient-to-r from-[var(--color-brand-500)] to-[var(--color-profit)] rounded-3xl blur opacity-20 group-hover:opacity-30 transition duration-1000" />
            <div className="relative p-10 bg-[#0A0D12] border border-white/10 rounded-3xl text-center space-y-8">
              <div className="w-20 h-20 rounded-2xl bg-white/[0.03] flex items-center justify-center mx-auto border border-white/5">
                <Award size={40} className={level.color} />
              </div>

              <div>
                <p className="text-[10px] font-bold uppercase tracking-widest text-[var(--color-text-muted)] mb-2">Simulated Score</p>
                <div className="text-7xl font-black tracking-tighter text-white">{score}</div>
              </div>
              ...
              <div className="space-y-4 pt-4 border-t border-white/5">
                <div className="flex items-center justify-between text-xs">
                  <span className="text-[var(--color-text-muted)]">Weighted Profit Factor</span>
                  <span className="text-white font-mono">{(score / 100).toFixed(2)}</span>
                </div>
                <div className="flex items-center justify-between text-xs">
                  <span className="text-[var(--color-text-muted)]">Institutional Standing</span>
                  <span className="text-white font-bold">{score >= 80 ? "EXCEPTIONAL" : score >= 60 ? "STABLE" : "IMPROVEMENT NEEDED"}</span>
                </div>
              </div>

              <p className="text-[9px] font-bold uppercase tracking-widest text-[var(--color-text-muted)]">
                FGC Certification isn&apos;t just about profit. It&apos;s about risk-adjusted consistency.
              </p>
            </div>
          </div>
        </div>
