...
const testimonials: Testimonial[] = [
  {
    quote:
      "I spent $3,000 on trading courses and signals before FGC. This is the first program that actually tested whether I could trade, not just whether I could watch videos.",
    name: "R.K.",
    role: "Forex Trader",
  ...
  {
    quote:
      "The certification scoring changed everything for me. When I saw my discipline score was 4/10, I understood why I kept blowing accounts. FGC showed me the problem in numbers, not opinions.",
    name: "S.M.",
    role: "Crypto Trader",
  ...
  },
  {
    ...
    statusColor: "text-[var(--color-profit)]",
  },
  {
    quote:
...
function TestimonialCard({ testimonial, index }: { testimonial: Testimonial; index: number }) {
  ...
  return (
    <motion.div
      ...
      className="p-8 md:p-10 bg-white/[0.03] border border-white/5 rounded-3xl space-y-6 hover:border-white/10 transition-all duration-300 group"
    >
      {/* Quote */}
      <p className="text-sm md:text-[15px] text-[var(--color-text-secondary)] leading-relaxed italic">
      ...
      <div className="flex items-center justify-between pt-4 border-t border-white/5">
        <div className="flex items-center gap-3">
          {/* Avatar placeholder */}
          <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[var(--color-brand-500)]/30 to-[var(--color-brand-600)]/10 border border-white/10 flex items-center justify-center">
            <span className="text-xs font-bold text-[var(--color-brand-400)]">
        ...
        <div
          className={cn(
            "px-3 py-1.5 rounded-lg bg-white/5 text-[9px] font-bold uppercase tracking-widest",
            testimonial.statusColor || "text-[var(--color-text-muted)]"
          )}
        >
          {testimonial.status}
        </div>
