...
/**
 * LURNAVA CURRICULUM LEVEL 1 - PLANNING TOKENS & STYLE SYSTEM
 */
export const UI = {
  navy: "#071B36",
  teal: "#0D9488",
...
/**
 * Institutional Frame Container for Module 1.5 Synthesis Visuals
 */
export const InstitutionalFrame = ({ 
  children, 
  label, 
  status = "SYNTHESIS STANDBY",
  id = "LN-1.5-SYNT"
}: { 
  ...
  <div className="w-full h-full min-h-[550px] bg-[#F8FBFC] border border-[#E2E8F0] rounded-[2rem] p-6 md:p-8 flex flex-col relative overflow-hidden shadow-sm">
    ...
    {/* Top Header Row */}
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 relative z-10 border-b border-slate-200 pb-4 shrink-0 text-left">
      <div className="flex flex-col">
        <div className="flex items-center gap-2 mb-1">
          <Lock size={10} className="text-[#071B36] animate-pulse" />
          <span className="text-[9px] font-black text-slate-400 uppercase tracking-[0.25em]">Synthesis Gate Shield</span>
        </div>
        <h4 className="text-base font-black text-[#071B36] uppercase tracking-tighter italic leading-tight">{label}</h4>
      </div>
      <div className="flex items-center gap-3">
        <div className="flex flex-col items-end">
          <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Gate Ref</span>
          <span className="text-[10px] font-mono text-[#071B36] font-bold">{id}</span>
        </div>
        <div className="w-px h-6 bg-slate-200" />
        <div className="flex items-center gap-2 px-3 py-1.5 bg-white border border-slate-200 rounded-lg shadow-sm">
          <div className="w-1.5 h-1.5 rounded-full bg-[#071B36] animate-pulse" />
    ...
    <div className="mt-6 pt-4 border-t border-slate-200 flex items-center justify-between shrink-0">
       <div className="flex gap-4">
          <div className="flex items-center gap-1.5">
            <div className="w-1 h-1 bg-[#071B36] rounded-full" />
            <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Synthesis Lock Active</span>
          </div>
       </div>
       <div className="flex items-center gap-1.5">
          <Fingerprint size={10} className="text-slate-300" />
          <span className="text-[8px] font-mono text-slate-300 uppercase tracking-tighter">LN-SYNT-M1.5-V1</span>
       </div>
    </div>
  </div>
);
...
/**
 * Mentor Insight / Guardrails Component
 */
       ...
       <div className="flex-1">
          ...
          {analogy && (
            <p className="mt-1.5 text-[11px] text-slate-500 italic leading-snug">
              <span className="font-black uppercase text-[9px] mr-2 text-slate-600 not-italic">Synthesis Analogy:</span>
              {analogy}
            </p>
          )}
       </div>
