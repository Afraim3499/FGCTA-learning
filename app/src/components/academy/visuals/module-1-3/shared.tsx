...
/**
 * Institutional Frame Container for Module 1.3 Target Visuals
 */
export const InstitutionalFrame = ({ 
  children, 
  label, 
  status = "TARGET STANDBY",
  id = "LN-1.3-TRGT"
}: { 
    ...
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 relative z-10 border-b border-slate-200 pb-4 shrink-0 text-left">
      <div className="flex flex-col">
        <div className="flex items-center gap-2 mb-1">
          <Target size={10} className="text-teal-600 animate-pulse" />
          <span className="text-[9px] font-black text-slate-400 uppercase tracking-[0.25em]">Target Coordinate Shield</span>
        </div>
    ...
    <div className="mt-6 pt-4 border-t border-slate-200 flex items-center justify-between shrink-0">
       <div className="flex gap-4">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Target Lock Active</span>
          </div>
       </div>
       <div className="flex items-center gap-1.5">
          <Fingerprint size={10} className="text-slate-300" />
          <span className="text-[8px] font-mono text-slate-300 uppercase tracking-tighter">LN-TRGT-M1.3-V1</span>
       </div>
    </div>
  </div>
);
       ...
       <div className="flex-1">
          ...
          {analogy && (
            <p className="mt-1.5 text-[11px] text-slate-500 italic leading-snug">
              <span className="font-black uppercase text-[9px] mr-2 text-teal-600 not-italic">Target Analogy:</span>
              {analogy}
            </p>
          )}
       </div>
