...
export function RecentActivityCard({ activities }: RecentActivityCardProps) {
  ...
  const getIcon = (type: string) => {
    switch(type) {
      ...
      case "start": return <Play className="w-4 h-4 text-[var(--ln-text-muted)]" />;
      case "review": return <Eye className="w-4 h-4 text-[var(--ln-text-muted)]" />;
      default: return <CheckCircle2 className="w-4 h-4 text-[var(--ln-text-muted)]" />;
    }
  };
  ...
  const getBgClass = (type: string) => {
    switch(type) {
      ...
      default:
        return "bg-slate-50 border border-[var(--ln-border)]";
    }
  };
  ...
  return (
    <div className="bg-white rounded-3xl p-5 border border-[var(--ln-border)] shadow-sm flex flex-col h-full hover:shadow-md transition-all">
      <div className="flex items-center justify-between mb-4">
        ...
        <Clock className="w-4 h-4 text-[var(--ln-text-muted)]" />
      </div>
        ...
        {activities.length > 0 ? (
          activities.map((act) => (
            <div key={act.id} className="flex items-center gap-4">
              ...
              <p className="text-[10px] text-[var(--ln-text-muted)] font-medium shrink-0">{act.timeAgo}</p>
            </div>
          ))
        ) : (
          <div className="flex flex-col items-center justify-center h-full text-[var(--ln-text-muted)] space-y-2 py-8">
