...
export const goldCards: LessonCard[] = [
  {
    ...
    body: "Before trusting a Gold move, you must locate XAU/USD through reaction zones, prior highs/lows, wick behavior, sweep areas, macro context, higher-timeframe location, close quality, and follow-through. This is not about drawing simple support and resistance lines. It is about understanding that a candlestick's information value is entirely determined by its coordinates. A massive green candle is meaningless if it prints in the middle of range noise, while a tiny doji at a weekly sweep area carries high structural information. By mapping these dimensions first, you filter out random price noise and read XAU/USD with clear context.",
    context: {
      keyTerms: [
        { term: "Gold Location", definition: "The specific structural coordinate where XAU/USD is trading relative to key reaction zones, prior highs/lows, and value boundaries." },
  ...
  },
  {
    ...
    body: "Gold does not react to magic, thin lines. Instead, XAU/USD responds to reaction zones—broad areas on the chart where price has previously changed behavior. When price returns to a reaction zone, the learner watches how it behaves: does price reject, accept, overshoot and return inside, or follow through? A reaction zone is not an instruction to buy or sell. It is simply a place of interest where price behavior becomes highly informative.",
    context: {
      keyTerms: [
        ...
        { term: "Acceptance Base", definition: "When price closes cleanly inside or beyond a zone and builds a new consolidation floor, confirming expansion." }
      ],
  ...
  },
  {
    ...
    body: "Gold is highly volatile and can print large, fast candles even when there is no real structural transition. When XAU/USD moves in the middle of a range, away from the upper and lower reaction zones, the reading quality of these candles is heavily degraded. This middle zone represents value equilibrium, where buyers and sellers swap orders without a clear advantage. Large candles here look important, but they are frequently traps that lead to immediate chop.",
    context: {
      keyTerms: [
        { term: "Range Midpoint", definition: "The center of a trading range where supply and demand are in equilibrium." },
        { term: "Equilibrium Zone", definition: "An area of fair value where price chops back and forth without institutional urgency." },
  ...
  },
  {
    ...
    body: "Prior daily or weekly highs and lows are crucial coordinates. Because these landmarks are highly visible, they act as magnets for resting order blocks and stop-losses. When XAU/USD returns to a prior high or low, you do not predict a reversal. Instead, you wait for price to test the coordinate. The Lurnava reader checks whether price wicks past it and returns inside, or accepts the area by closing cleanly beyond it.",
    context: {
      keyTerms: [
        { term: "Prior Swing Landmarks", definition: "Recent daily or weekly highs and lows where major price reversals occurred." },
        { term: "Liquidity Pool", definition: "A concentration of resting orders (stop-losses or buy/sell stops) sitting just beyond key landmarks." },
        { term: "Breakout Crossing", definition: "The temporary moment price crosses a prior high or low before close quality is confirmed." },
  ...
  },
  {
    ...
    context: {
      keyTerms: [
        { term: "Wick Evidence", definition: "The thin line on a candlestick representing intra-candle price rejection at specific coordinates." },
  ...
  },
  {
    ...
    body: "Gold is highly sensitive to macroeconomic events such as inflation data (CPI), jobs reports (NFP), interest rate decisions, and Treasury yield shifts. During these announcements, Gold will print massive, volatile candles. A macro-driven spike can overshoot a reaction zone by 20 dollars in seconds. To read these locations, you must practice extreme patience. Let the event noise settle, and check where the candle closes relative to the reaction zone.",
    context: {
      keyTerms: [
        { term: "Macro Location", definition: "Price coordinates that become active or distorted during high-impact economic news releases." },
  ...
  },
  {
    ...
    context: {
      keyTerms: [
        ...
        { term: "Reading Priority", definition: "Giving higher-timeframe location coordinates priority over lower-timeframe candle patterns." }
      ],
  ...
  },
  {
    ...
    context: {
      keyTerms: [
        ...
        { term: "Close Quality", definition: "The location of a candle's close relative to range highs/lows rather than its own range." },
  ...
  },
  {
    ...
    body: "Congratulations! You have completed the Module 0.8 Gold Roadway track. You have built a critical filter: before trusting a Gold move, locate XAU/USD through reaction zones, prior highs/lows, wick behavior, sweep areas, macro context, higher-timeframe location, close quality, and follow-through. By filtering out range midpoints, watching prior swing landmarks, overlaying higher-timeframe boundaries, and waiting for close quality and follow-through, you read XAU/USD with institutional objectivity.",
    context: {
      keyTerms: [
